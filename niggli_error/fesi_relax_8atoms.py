...
if __name__ == "__main__":
    ...
    calc = GPAW(mode=PW(e_cut), nbands = nbands,
                xc='PBE', spinpol=True, kpts=[2,2,2] ,symmetry = {'do_not_symmetrize_the_density': True},
    ...
    if os.path.isfile('./' + system_name + '.gpw'):
        ...
        bulk_mat = traj[-1]
        bulk_mat.set_calculator(calc)
        print('Restarting with atoms from trajectory')
    ...
    if density_number == 5.4:
    	...
    	save_atoms(bulk_mat_initial, e_cut, nbands, density_number, smear, initial_magmom, si_concentration, system_id, 1, final_db)
